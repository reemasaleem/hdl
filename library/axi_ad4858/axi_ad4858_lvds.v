...
module axi_ad4858_lvds #(
  ...
  always @(posedge clk) begin
    ch_7_index <= adc_crc_enable ? 1 : 0;
    ch_6_index <= adc_crc_enable ? 2 : 1;
    ch_5_index <= adc_crc_enable ? 3 : 2;
    ch_4_index <= adc_crc_enable ? 4 : 3;
    ch_3_index <= adc_crc_enable ? 5 : 4;
    ch_2_index <= adc_crc_enable ? 6 : 5;
    ch_1_index <= adc_crc_enable ? 7 : 6;
    ch_0_index <= adc_crc_enable ? 8 : 7;
  ...
  always @(posedge clk) begin
    if (adc_valid_init_d == 1'b1) begin
      crc_cnt <= crc_data_length;
      run_crc <= (adc_crc_enable == 1) ? 1'b1 : 1'b0;
    end else begin
  ...
  assign crc_reset = adc_valid_init_d;
